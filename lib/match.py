...
def setup_match(match, f1_human, f2_human, reset=True):
    ...
    if f2_human:
        fighter = fighter_loader.load_fighter("ik_guy", IK_Guy,
                                              ([137, 64, 54], [255, 255, 255]))
    else:
        fighter = fighter_loader.load_fighter("simple_ik_guy", AIBlocker,
                                              ([137, 64, 54], [255, 255, 255]))
        fighter.difficulty = DEFAULT_CPU_DIFFICULTY
        if f1_human == False:
...
class Match(GameState):
    ...
    def reset(self):
        ...
        self.world.view.set_background(
            gfx.load_image("images/bkgd-crop.png")[0])
    ...
    def add_fighter(self, fighter):
        ...
        if len(self.fighter) == 1:
            self.controller.append(
                engine.MappedInput(fighter, engine.p1_key_def))
        else:
            self.controller.append(
                engine.MappedInput(fighter, engine.p2_key_def))
