...
def load_fighter(name, class_name=None, colorize=None):
    ...
    if colorize is not None:
        color_swap = [pygame.Color(*i) for i in colorize]
...
def load_frame(path, d, klass=AnimationFrame):
    ...
    name = d['file']
    mask_file = os.path.join(path, name[:name.index(".")] + "-mask" + name[
                                                                      name.index(
                                                                          "."):])
    get_masks_from_image(mask_file)

    if color_swap is not None:
    ...
    try:
        for x in range(1, 6):
            ...
            if d.has_key(section_name):
                ...
                box = CollisionBox(hb[:2],
                                   (hb[2] - hb[0] + 1, hb[3] - hb[1] + 1))
